...
from calibration.calib import  board_points, detect_board, detect_apriltags
...
# input images
input_dir = "data/stereo/captures/raiz_apriltags"
...
for left_file_name, right_file_name in zip(
        ...
        left_object_points, left_image_points = detect_apriltags(left_image_rectified, tag_size_mm,tag_family)
        right_object_points, right_image_points = detect_apriltags(right_image_rectified, tag_size_mm, tag_family)
        ...
        ret, rvec, tvec = cv2.solvePnP(
            np.concatenate(left_object_points),
            np.concatenate(left_image_points),
            left_K,
            left_dist,
        ...
        #point_cloud, colors = filter_point_cloud(point_cloud, colors)
